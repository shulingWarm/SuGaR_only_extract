...
#用于处理输入数据的操作，这里直接弄一个简单的结构体来保存相关的输入了
class InputStruct:
    def __init__(self):
        # 把3D高斯的细节投影到Mesh上
        self.project_mesh_on_surface_points = False
        # 三维高斯的场景，这个后面再具体写内容
        self.scene_path = "/media/zzh/data/temp/south-building/"
        # checkpoint 这个或许是没用的 其实这个东西就是3D Gaussian在训练过程中的一个中间保存点
        self.checkpoint_path = "/media/zzh/data/temp/sugar_input/"
        # 读取哪个迭代周期的数据，这对于这个独立的子任务似乎没有什么意义
        self.iteration_to_load = 7000
        # 对模型使用eval状态，这可能指的是一种非训练的状态吧
        self.eval = True
        # 粗糙的mesh,这个可能是算法的一些前导数据
        self.coarse_model_path = ""
        # 提取mesh的层次，其实不知道是干嘛用的
        self.surface_level = 0.6
        # Mesh里面的节点的个数，未知的数据项目
        self.decimation_target = 1000000
        # 这个才更像是输出mesh
        self.mesh_output_dir = "/media/zzh/data/temp/sugar.ply"
        # 生成mesh时的范围
        self.bboxmin = None
        self.bboxmax = None
        # 把box放到中间，应该只是为了对输出的mesh添加一个全局的移位
        self.center_bbox = True
        # 下面三个选项是写死成False的，不知道它们具体是做什么用的
        self.use_centers_to_extract_mesh = False
        self.use_marching_cubes = False
        self.use_vanilla_3dgs = True
        # 默认使用的gpu id
        self.gpu = 0
...
print('Calling extract_mesh_from_coarse_sugar')
extract_mesh_from_coarse_sugar(InputStruct())